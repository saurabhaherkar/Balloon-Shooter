...
class Balloon:
    def __init__(self, speed):
        ...
        self.color = random.choice([red, green, orange, purple, yellow, blue])
    ...
    def move(self):
        ...
        if (self.x + self.a) > width or self.a < 0:
            if self.y > height // 5:
                self.x -= self.speed * cos(radians(self.angle))
            else:
                self.reset()
...
def pointer():
    ...
    color = lightGreen
    for i in range(noBalloon):
        if onBalloon(balloon[i].x, balloon[i].y, balloon[i].a, balloon[i].b, pos):
            color = red
...
def lowerPlatform():
    pygame.draw.rect(display, darkGrey, (0, height - lowerBound, width, lowerBound))
